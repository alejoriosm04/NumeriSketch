...
def multiple_roots_graph(fun, xi, xf, root=None, png_path='static/graphs/multiple_roots_graph.png', svg_path='static/graphs/multiple_roots_graph.svg'):
    """Genera y guarda la gráfica de la función para el método de Raíces Múltiples."""
...
def multiple_roots(request):
    ...
    if request.method == 'POST':
        try:
            ...
            ddfx = request.POST.get('ddfx', '').strip()
            ...
            if not dfx:
                ...
                dfx = str(dfx_sympy)
            if not ddfx:
                ddfx_sympy = sp.diff(fx_sympy, x_sym, 2)
                ddfx = str(ddfx_sympy)

            # Verificar las derivadas
            try:
                eval(dfx, {"x": x0, **safe_math()})
                eval(ddfx, {"x": x0, **safe_math()})
            except Exception as eval_error:
                raise ValueError(f"Expresión no válida en las derivadas: {eval_error}")
            ...
            while iteration < niter:
                ...
                ddf_value = eval(ddfx, {"x": x0, **safe_math()})

                denominator = (df_value ** 2) - (f_value * ddf_value)
                if denominator == 0:
                    derivative_zero = True
                    context['msg'] = [f"División por cero en x = {x0}, el método no puede continuar. La derivada en el punto debe ser diferente de cero."]
                    break

                # Fórmula del método de Raíces Múltiples
                x1 = x0 - (f_value * df_value) / denominator
            ...
            graph_paths = multiple_roots_graph(fx, xi, xf, root)
